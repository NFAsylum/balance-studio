...
def _reset(monkeypatch):
    apimain._rate_state.clear()
    monkeypatch.setattr(apimain, "_RATE_LIMIT_PER_MIN", 1000)  # generous unless a test lowers it
    yield
    apimain._rate_state.clear()


def test_write_without_key_is_401_when_key_configured(monkeypatch, client):
    monkeypatch.setattr(apimain, "_API_KEY", "secret")
...
def test_no_key_configured_allows_writes(monkeypatch, client):
    monkeypatch.setattr(apimain, "_API_KEY", None)  # dev mode
    assert client.post(_WRITE, json=_BODY).status_code == 404


def test_rate_limit_returns_429(monkeypatch, client):
    monkeypatch.setattr(apimain, "_API_KEY", None)
    monkeypatch.setattr(apimain, "_RATE_LIMIT_PER_MIN", 2)
    assert client.post(_WRITE, json=_BODY).status_code == 404
    assert client.post(_WRITE, json=_BODY).status_code == 404
